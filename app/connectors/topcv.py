...
class TopCVProvider(AuthorizedApiProvider):
    name = "topcv"
    ...
    async def apply(self, request: ProviderApplicationRequest) -> ProviderApplicationResult:
        ...
        payload["job_id"] = request.job_source_record_id
        response = dict(result.response_payload)
        external_id = result.external_application_id or f"topcv-{request.job_source_record_id}-{request.candidate_id}"
